...
from email_helpers import connect_to_email, fetch_unread_emails, send_email, mark_as_read
...
def process_emails_and_create_ui(df):
    ...
    for idx, row in df.iterrows():
        response_json = analyze_email(row['Content'], client, ASSISTANT_ID, MAX_FETCH_ATTEMPTS)
        ...
        with st.expander(response_json['summary'], icon=importance_emoji):
            if response_json['importance'] == 0:
                ...
                if st.button(f"Send ✉️", key=f"send_{idx}"):
                    if send_email(row['From'], row['Subject'], draft_response, EMAIL_ACCOUNT, PASSWORD):
...
if (st.sidebar.button('Go!') and check_rate_limit(SESSION_TIMEOUT, MAX_FETCHES_PER_SESSION)) or 'already_started' in st.session_state:
